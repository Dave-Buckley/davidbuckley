...
export default function ProgressPage() {
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-16 space-y-10">
      ...
      {next ? (
        <section className="border-l-4 border-amber-700 border-y border-r border-amber-200 bg-amber-50 rounded-sm p-6 space-y-4">
          <div className="space-y-1">
            <p className="text-xs uppercase tracking-wider text-amber-900 font-medium">
              Next action · Phase {next.phase.number} · {next.phase.name}
            </p>
            <h2 className="text-xl font-medium text-neutral-900">{next.task.text}</h2>
            {next.task.reqId && (
              <p className="text-xs text-neutral-600">Requirement: {next.task.reqId}</p>
            )}
          </div>
          ...
          {next.task.url && (
            <div className="pt-2">
              <a
                href={next.task.url}
                className="inline-block text-sm font-medium border border-amber-700 bg-amber-700 text-white px-4 py-2 rounded-sm hover:bg-amber-800 hover:border-amber-800 transition-colors"
              >
                Open resource →
              </a>
            </div>
          ...
          <div className="border-t border-amber-200 pt-4 text-sm text-neutral-800 space-y-2">
            <p className="font-medium text-neutral-900">Session workflow</p>
            <ol className="list-decimal list-inside space-y-1">
              <li>
                Run{' '}
                <code className="text-xs bg-white border border-amber-200 px-1 py-0.5 rounded">
                  npm run study:start
                </code>{' '}
              ...
              <li>
                ...
                , summarise what shipped, commit and push.
              </li>
            </ol>
          </div>
        </section>
          ...
          <ul className="space-y-4">
            {activePhases.map((phase) => {
              ...
              return (
                <li
                  key={phase.slug}
                  className="border border-amber-200 rounded-sm p-5 space-y-3 bg-white"
                >
                  ...
                  <ProgressBar percent={prog.percent} variant="active" />
                    ...
                    <ul className="text-sm space-y-1 pt-1">
                      {remaining.map((t, i) => (
                        <li key={i} className="text-neutral-700">
                          ...
                          {t.reqId && <span className="text-neutral-400"> ({t.reqId})</span>}
                        </li>
                      ))}
                    </ul>
                  )}
